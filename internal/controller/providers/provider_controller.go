...
func (r *ProviderReconciler) Reconcile(ctx context.Context, req mcreconcile.Request) (ctrl.Result, error) {
	...
	metrics.ReconcileTotal.WithLabelValues(ProviderControllerName, labelResult).Inc()
	return result, err
}
...
func NewProviderReconciler(mgr mcmanager.Manager, runtimeClient client.Client, operatorCfg *config.OperatorConfig, commonCfg *pmconfig.CommonServiceConfig) (*ProviderReconciler, error) {
	...
	if operatorCfg.Providers.Subroutines.Providers.Workspace.Enabled {
	...
	if operatorCfg.Providers.Subroutines.Providers.Kubeconfig.Enabled {
